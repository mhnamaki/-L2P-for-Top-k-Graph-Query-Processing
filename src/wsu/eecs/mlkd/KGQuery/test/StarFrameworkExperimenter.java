...
public class StarFrameworkExperimenter {
	...
	private static String shouldCheckQueries = null;
	...
	public static void main(String[] args) throws Exception {
		...
		File databaseFile = new File(MODELGRAPH_DB_PATH);
		GraphDatabaseService knowledgeGraph = new GraphDatabaseFactory().newEmbeddedDatabase(databaseFile);
		...
		// for (int querySize = querySizeFrom; querySize <= querySizeTo;
		// querySize++) {
					...
					for (Integer k : k_s) {
						...
						try (Transaction tx1 = queryGraph.beginTx()) {
							try (Transaction tx2 = knowledgeGraph.beginTx()) {
								...
								StarFramework starFramework2 = new StarFramework(queryGraph, knowledgeGraph, k, alpha,
										levenshtein);
								...
								printResults(queryIndex, 0, finalResults.size(), k, diffTime, exp, bw, bwTime,
										finalResults);
...
//								}
	...
	public static void finalizeMethod(StarFramework starFramework2) {
		...
		for (GraphResult gr : starFramework2.finalResultsArrayList) {
			for (Integer starQueryIndex : gr.starQueryIndexDepthMap.keySet()) {
				if (gr.starQueryIndexDepthMap.get(starQueryIndex) > maxAnswerDepthStarQueryMap.get(starQueryIndex)) {
					maxAnswerDepthStarQueryMap.put(starQueryIndex, gr.starQueryIndexDepthMap.get(starQueryIndex));
				}
			}
		}
		...
		for (Integer starQueryIndex : maxAnswerDepthStarQueryMap.keySet()) {
			System.out.println("In Answers: depthOfDigging for starQuery with nodeIndex: " + starQueryIndex + " is "
