...
public class BoostingLearningV3 {
	...
	public AnyTimeStarFramework beamSearchSF;
	...
	public void initialize(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; i++) {
			...
			} else if (args[i].equals("-maxStepSize")) {
				maxStepSize = Integer.parseInt(args[++i]);
			}
		...
		System.out.println("after training first cls");
		...
		System.out.println("after training first reg");
	...
	private void searchOverQueries(int iteration, HashSet<Integer> queriesSet, Predictor classifierPredictor,
		...
		for (File file : CommonFunctions.fileInTheDirfinder(queryFileDirectory)) {
			...
			for (QueryFromFile queryFromFile : queriesFromFile) {
				...
				if (maxStepSize > 0 && oracleSelectionSteps.size() > maxStepSize) {
					System.out.println(queryIndex + " not run due to being long tail");
					queryGraph.shutdown();
					queryGraph = null;
					System.gc();
				...
				try (Transaction tx1 = queryGraph.beginTx()) {
					try (Transaction tx2 = knowledgeGraph.beginTx()) {
						...
						for (int exp = 0; exp < numberOfSameExperiment; exp++) {
							...
							Double end_time = (double) System.nanoTime();
						...
						// ;
						double rankCoeff = 0.352233;
						double timeCoeff = 0.312991;
						double depthCoeff = 0.25264;
						...
						finalizeMethod(starFramework2, maxAnyTimeAnswerDepthStarQueryMap);
						...
						boolean askActionFromPolicies = true;
						mlSF = getNewStarFrameworkInstance();

						beamSearchSF = getNewStarFrameworkInstance();
						...
						// Dummy.DummyFunctions.whatsGoingon(mlSF);
						// System.out.println();
						...
						while (!mlSF.anyTimeAlgorithmShouldFinish()) {
							...
							AnyTimeStarFramework tempBeamSearch = beamSearchSF.copy(queryGraph, knowledgeGraph);
							BeamSearchV3 beamSearch = new BeamSearchV3(queryGraph, knowledgeGraph, beamSize, 20,
									WhichOracle.macroSingleAction, this, neighborIndexingInstance, cacheServer,
									maxAnyTimeAnswerDepthStarQueryMap, trueRank, numberOfQNodes, numberOfQRelationships,
									MinObjectiveFunction.TotalTime, rankCoeff, timeCoeff, depthCoeff,
									finalDifferenceTime, infoHolders[0], mlSF, tempBeamSearch);

							if (level % 500 == 0)
								System.out.println("at level:" + level + "sequenceForDebug:"
										+ beamSearch.lastBeamSearchNode.sequenceForDebug);

							// Dummy.DummyFunctions.whatsGoingon(beamSearch.lastBeamSearchNode.starframework);
							...
							int paSelectedFromOracle = getOracleSelection(numberOfStars,
									beamSearch.lastBeamSearchNode.sequenceForDebug);

							// if (paSelectedFromOracle <= 0) {
							// break;
							// }
							...
							askActionFromPolicies = askActionsFromPolicies(beta);
							// System.out.println("askActionFromPolicies: " +
							// askActionFromPolicies);
							...
							paSelected = askActionFromPolicies ? (int) predicts[0] : paSelectedFromOracle;

							// System.out.println("selection:
							// askActionFromPolicies?" + askActionFromPolicies
							// + ", paSelection:" + (int) predicts[0] + ",
							// paOracleSelection:" + paSelectedFromOracle
							// + " seq:" +
							// beamSearch.lastBeamSearchNode.sequenceForDebug);
							...
							int paOracleExpansion = getOracleExpansion(numberOfStars,
									beamSearch.lastBeamSearchNode.sequenceForDebug);
							...
							paExpansion = askActionFromPolicies ? paExpansion : paOracleExpansion;

							// System.out.println("expansion:
							// askActionFromPolicies?" + askActionFromPolicies
							// + ", paExpansion:" + paExpansion + ",
							// paOracleExpansion:" + paOracleExpansion
							// + " seq:" +
							// beamSearch.lastBeamSearchNode.sequenceForDebug);
	...
	private boolean askActionsFromPolicies(double beta) {
	...
	private int getOracleSelection(int numberOfStars, String sequenceForDebug) {
		// < 1: 1 >, < 2: 1 >, < 3: 1 >, <3: 20>, <2: 140>, <3: 20>
		// < 1: 1 >, < 2: 1 >, < 3: 20 >, <2: 140>,
		// < 1: 1 >, < 2: 167 >, < 3: 20 >, <3: 20>
		String selection = sequenceForDebug.split(",")[numberOfStars].replace("<", "").replace(">", "").split(":")[0]
				.trim();

		int retValue = 0;
		try {
			retValue = Integer.parseInt(selection);
		} catch (Exception exc) {
			// exc.printStackTrace();
			// throw exc;
			// System.out.println();
	...
	private int getOracleExpansion(int numberOfStars, String sequenceForDebug) {
		// < 1: 1 >, < 2: 1 >, < 3: 1 >, <3: 20>, <2: 140>, <3: 20>
		// < 1: 1 >, < 2: 1 >, < 3: 20 >, <2: 140>,
		// < 1: 1 >, < 2: 167 >, < 3: 20 >, <3: 20>
		int retValue = 50;
		try {
			String[] splittedData = sequenceForDebug.split(",");
			if (splittedData.length > (numberOfStars + 1)) {

				String expansion = splittedData[numberOfStars].replace("<", "").replace(">", "").split(":")[1].trim();

				retValue = Integer.parseInt(expansion);
			}
	...
	public static void main(String[] args) throws Exception {
		BoostingLearningV3 boostingLearning = new BoostingLearningV3();
	...
	public void finalizeMethod(AnyTimeStarFramework starFramework2,
