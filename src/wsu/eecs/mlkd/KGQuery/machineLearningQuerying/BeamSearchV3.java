...
public class BeamSearchV3 {
	...
	public BoostingLearningV3 beamSearchRunner;
	...
	AnyTimeStarFramework mlSF;
	AnyTimeStarFramework tempBeamSearch;
	...
	public BeamSearchV3(GraphDatabaseService queryGraph, GraphDatabaseService knowledgeGraph, int beamSize, int delta,
			WhichOracle oracle, BoostingLearningV3 beamSearchRunner, NeighborIndexing neighborIndexingInstance,
			...
			double timeCoeff, double depthCoeff, double SFTotalTime, InfoHolder infoHolder, AnyTimeStarFramework mlSF,
			AnyTimeStarFramework tempBeamSearch) throws Exception {
		...
		this.mlSF = mlSF;
		this.tempBeamSearch = tempBeamSearch;
		maxDelta = delta * 10;

		maxDistance = getMaxPossibleDistance();
		initialize(queryGraph, knowledgeGraph, neighborIndexingInstance, cacheServer, mlSF);

		// beamSearch();
	}

	public void initialize(GraphDatabaseService queryGraph, GraphDatabaseService knowledgeGraph,
			NeighborIndexing neighborIndexingInstance, CacheServer cacheServer, AnyTimeStarFramework mlSF2)
			throws Exception {
		...
		//DummyFunctions.whatsGoingon(startNode.starframework);
	...
	private BeamSearchNode rootHandling(GraphDatabaseService queryGraph, GraphDatabaseService knowledgeGraph,
		...
		for (Integer starQueryNodeIndex : calcTreeNodeStarQueryMaxDepthMap.keySet()) {
			int fetchedAmount = mlSF.calcTreeStarQueriesNodeMapBySQMLIndex
					.get(DummyFunctions.getSQIndexByCalcNodeIndexAndNumberOfStars(starQueryNodeIndex,
							calcTreeNodeStarQueryMaxDepthMap.size())).data.depthOfDigging;
			rootInsideArr
					.add(new StarQueryIndexWithNumberOfPartialAnswerToBeFetched(starQueryNodeIndex, fetchedAmount));

			sequenceForDebug += "< " + DummyFunctions.getSQIndexByCalcNodeIndexAndNumberOfStars(starQueryNodeIndex,
					calcTreeNodeStarQueryMaxDepthMap.size()) + ": " + fetchedAmount + " >, ";
		}
		...
		startNode.starframework = tempBeamSearch;// beamSearchRunner.getNewStarFrameworkInstance();

		//System.out.println("start node: " + startNode.sequenceForDebug);
	...
	public BeamSearchNode beamSearch(GraphDatabaseService queryGraph, GraphDatabaseService knowledgeGraph,
		...
		HashSet<BeamSearchNode> firstChildrenOfInitialState = new HashSet<BeamSearchNode>();
		...
		while (!beamNodesSet.isEmpty()) {
			...
			if (!fistTime && firstChildrenOfInitialState.size() == 0)
				System.err.println("you made them null!");

			HashSet<BeamSearchNode> firstContributingAncestors = new HashSet<BeamSearchNode>();
			BeamSearchNode selectedFromFirstLevel = null;
			for (BeamSearchNode beamSearchNode : beamNodesSet) {
				if (firstChildrenOfInitialState.contains(beamSearchNode.firstAncestor)) {
					firstContributingAncestors.add(beamSearchNode.firstAncestor);

					// System.out.println("lvl: " + beamSearchNode.level);
					// System.out.println("1stAncestor:
					// "+beamSearchNode.firstAncestor);
					// System.out.println("currentNode:
					// "+beamSearchNode.sequenceForDebug);
					// System.out.println();

					selectedFromFirstLevel = beamSearchNode;
				}
			}

			// System.out.println("firstContributingAncestors size: " +
			// firstContributingAncestors.size());
			if (firstContributingAncestors.size() == 1) {
				//DummyFunctions.whatsGoingon(selectedFromFirstLevel.starframework);
				return selectedFromFirstLevel;
			}
			...
			for (BeamSearchNode node : beamNodesSet) {
				// if ((node.level % 100) == 0) {
				// System.out.println("node.level:" + node.level + " target
				// distance: " + node.targetDistance);
				// }
			...
			while ((!set.isEmpty()) && (beamNodesSet.size() < beamSize)) {
				...
				if (betterState.parentNode != null) {
					...
					if (!firstChildrenOfInitialState.contains(betterState.parentNode.parentNode)) {
						betterState.parentNode.parentNode = null; // solving
																	// memory
						// leakage
						// problem
					}
				...
				if (betterState.level == 1) {
					firstChildrenOfInitialState.add(betterState);
				}
				// }
			}
			...
			for (BeamSearchNode uselessNode : set) {
				...
				if (uselessNode.parentNode != null
						&& !firstChildrenOfInitialState.contains(uselessNode.parentNode.parentNode)) {
					uselessNode.parentNode.parentNode = null;
				}
				if (!firstChildrenOfInitialState.contains(uselessNode.parentNode)) {
					uselessNode.parentNode = null;// solving memory leakage
													// problem
				}
	...
	private double runIncrementally(GraphDatabaseService queryGraph, GraphDatabaseService knowledgeGraph,
		...
		if (successor.starframework == null && successor.parentNode != null) {
			...
			successor.starframework = tempBeamSearch;// beamSearchRunner.getNewStarFrameworkInstance();
		}
			...
			if (oracle == WhichOracle.macroSingleAction) {
				if (node.starframework.calcTreeNodeMap
					...
					for (Integer curDelta : deltas) {
						...
						if (node.level == 1) {
							successorNode.firstAncestor = node;
						} else if (node.level > 1) {
							successorNode.firstAncestor = node.firstAncestor;
						}
