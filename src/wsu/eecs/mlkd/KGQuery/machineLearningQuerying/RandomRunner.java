...
public class RandomRunner {
				...
				try (Transaction tx1 = queryGraph.beginTx()) {
					try (Transaction tx2 = knowledgeGraph.beginTx()) {
						...
						if (anyTimeResultsSizeTemp == 0) {
							...
							int[] expansionOptions = new int[] { 20, 60, 100, 140, 180, 200 };

							File foutTime = new File(GName + "_randomAlgorithm.txt");
							...
							for (int exp = 0; exp < numberOfSameExperiment; exp++) {
								...
								int[] selectionOptions = new int[numberOfStars + 1]; // +HALT
																						// action
								...
								for (Integer starQNode : rndSF.calcTreeStarQueriesNodeMap.keySet()) {
									...
									depthJoinLevel = rndSF.calcTreeStarQueriesNodeMap.get(starQNode).levelInCalcTree
											- 1;
								...
								while (true) {
									...
									paSelected = rnd.nextInt(selectionOptions.length);
							...
							saveTheResults(queryIndex, bwMLTime, sf, finalSFDifferenceTime, rndSF, mlDifferenceTime, 0d,
									0d, 0d, 0d, numberOfClassifierCalls, expansions, infoHolders);
	...
	public static void main(String[] args) throws Exception {
		RandomRunner beamSearchRunner = new RandomRunner();
