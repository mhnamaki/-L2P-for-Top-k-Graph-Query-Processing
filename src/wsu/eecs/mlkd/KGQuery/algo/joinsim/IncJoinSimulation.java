...
public class IncJoinSimulation {
	...
	public boolean Init() throws IOException {
		...
		//System.out.println("Pattern graph size: " + pg.vertexSet().size());
		...
		for (gpm_node pn : pg.vertexSet()) {
			...
			if (ans.size() > maxs)
				maxs = ans.size();
			//System.out.print(pn.tag + ": " + ansvec.size() + "       ");
	...
	public double IsBSim(int idxmode, boolean opt) throws IOException {
		...
		//msize();
		for (String pn : matchSet.keySet()) {
			pg.simMatchSet.put(pn, matchSet.get(pn));
		}
		
		pg.interestingness = interestingnessCal();
		//System.out.println("interestingness = " + pg.interestingness);
	...
	public double interestingnessCal(){
		...
		return (pg.vertexSet().size() * matchAllNode.size() / (double)dg.nodes.size());
	}
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < time; i++) {
			...
			System.out.println("after construction: " + pg.vertexSet().size());
			System.out.println("Before simulation:" + pg.simMatchSet.get("0"));
