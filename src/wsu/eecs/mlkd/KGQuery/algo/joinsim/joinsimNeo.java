...
import wsu.eecs.mlkd.KGQuery.algo.joinsim.Neo4jGraph.MyRelationshipTypes;
import wsu.eecs.mlkd.KGQuery.algo.joinsim.gpm_distFWMatrix;
import wsu.eecs.mlkd.KGQuery.algo.joinsim.Neo4jGraph;
import wsu.eecs.mlkd.KGQuery.algo.joinsim.gpm_resultProcess;
...
//		System.out.println((pg == null) + " pg null check");
		...
		for (gpm_node pn : pg.vertexSet()) {
			...
			if(pn.tag.equals("0")){
				...
				HashSet<String> nodeDelete = new HashSet<String>();
				input.add("hasFamilyName");
				ans.addAll(propertyIndex.GetAnswer(input));
				for(String ansNodeIdString : ans){
					a1 = dg.GetNewNodesByRelationship(ansNodeIdString, "http://yago-knowledge.org/resource/hasWonPrize");
					
					if(a1.size() == 0)
						nodeDelete.add(ansNodeIdString);
				}
				ans.removeAll(nodeDelete);
				nodeDelete.clear();
				for(String ansNodeIdString : ans){
					a2 = dg.GetNewNodesByRelationship(ansNodeIdString, "http://yago-knowledge.org/resource/wasBornIn");
					if(a2.size() == 0)
						nodeDelete.add(ansNodeIdString);
				}
				System.out.println("nodeDelete size: " + nodeDelete.size());
				ans.removeAll(nodeDelete);
				
				System.out.println("ans size for node 0 = " + ans.size());
			}
			if(pn.tag.equals("1")){
				ans.addAll(matchSet.get("0"));
				System.out.println("ans size for node 1 = " + ans.size());
			}
			if(pn.tag.equals("2")){
				for(String id : matchSet.get("0"))
					ans.addAll(dg.GetNewNodesByRelationship(id, "http://yago-knowledge.org/resource/hasWonPrize"));
				System.out.println("ans size for node 2 = " + ans.size());
			}
			if(pn.tag.equals("3")){
				//ans.addAll(matchSet.get("2"));
				for(String id : matchSet.get("0"))
					ans.addAll(dg.GetNewNodesByRelationship(id, "http://yago-knowledge.org/resource/wasBornIn"));
				System.out.println("ans size for node 3 = " + ans.size());
			}
	...
	public int simJoin(gpm_node a, gpm_node b) {
		...
		boolean remv = true;
		int bound = (pg.getEdge(a, b)).e_bound;
		int color = ((gpm_edge)pg.getEdge(a, b)).e_color;
	...
	public Vector<Long> IsBSim(int idxmode, boolean opt) throws IOException {
		...
		// build up matrix
		if (idxmode == 1) {
			System.out.println("before matrix: " + time);
			if (dg.FS == null || dg.FS.d[0][0][0] == -1) {
				System.out.println("recompute matrix.");
				dg.FS = null;
				//dg.FS = new gpm_distFWMatrix(dg, true);
			}
			System.out.println("After matrix: " + System.currentTimeMillis());
			matrixtime = System.currentTimeMillis() - time;
			System.out.println("Matrix Construct time: " + matrixtime);
		}
	...
	public static void main(String[] args) throws IOException {
		...
		String filename1 = "/Users/qsong/Downloads/yago_query_3.db";
		String filename2 = "/Users/qsong/Downloads/YagoCores_graph.db";
