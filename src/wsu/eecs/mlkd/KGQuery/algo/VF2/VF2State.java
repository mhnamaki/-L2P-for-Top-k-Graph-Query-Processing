...
public class VF2State {
	...
	public VF2State(GraphDatabaseService modelGraph, GraphDatabaseService patternGraph) {
		...
		modelSize = DummyProperties.knowledgeSize;
		patternSize = DummyProperties.patternSize;

		// increasing the performance possible
		// GlobalGraphOperations globalOperation1 =
		// GlobalGraphOperations.at(modelGraph);
		// for (Node n : globalOperation1.getAllNodes()) {
		// modelSize++;
		// }

		// increasing the performance possible
		// GlobalGraphOperations globalOperation2 =
	...
	public void match(int n, int m) {
		...
		for (Relationship e : nTmp.getRelationships(Direction.INCOMING)) {
			if (in_1[(int) getUniqueManualIdByNode(e.getStartNode())] == -1) {
				in_1[(int) getUniqueManualIdByNode(e.getStartNode())] = depth; // update
																				// in_1
				if (!inM1((int) getUniqueManualIdByNode(e.getStartNode())))
					T1in.add((int) getUniqueManualIdByNode(e.getStartNode())); // update
																				// T1in
			}
		}
		...
		for (Relationship e : nTmp.getRelationships(Direction.OUTGOING)) {
			if (out_1[(int) getUniqueManualIdByNode(e.getEndNode())] == -1) {
				out_1[(int) getUniqueManualIdByNode(e.getEndNode())] = depth; // update
																				// out_1
				if (!inM1((int) getUniqueManualIdByNode(e.getEndNode())))
					T1out.add((int) getUniqueManualIdByNode(e.getEndNode())); // update
																				// T1out
			}
		}
		...
		for (Relationship e : mTmp.getRelationships(Direction.INCOMING)) {
			if (in_2[(int) getUniqueManualIdByNode(e.getStartNode())] == -1) {
				in_2[(int) getUniqueManualIdByNode(e.getStartNode())] = depth; // update
																				// in_2
				if (!inM2((int) getUniqueManualIdByNode(e.getStartNode())))
					T2in.add((int) getUniqueManualIdByNode(e.getStartNode())); // update
																				// T2in
			}
		}
		...
		for (Relationship e : mTmp.getRelationships(Direction.OUTGOING)) {
			if (out_2[(int) getUniqueManualIdByNode(e.getEndNode())] == -1) {
				out_2[(int) getUniqueManualIdByNode(e.getEndNode())] = depth; // update
																				// out_2
				if (!inM2((int) getUniqueManualIdByNode(e.getEndNode())))
					T2out.add((int) getUniqueManualIdByNode(e.getEndNode())); // update
																				// T2out
			}
		}
	}
	...
	public Node getNodeByProperty(GraphDatabaseService graph, int uniqueManualId) {
