...
public class AnyTimeStarFramework {
	...
	// top-k
	public HashMap<StarGraphQuery, HashMap<NodeWithValue, PriorityQueue<StarGraphResult>>> currentLatticeResultsOfStarkForGenerateNextBestMatchOfTheSQuery;

	HashMap<StarGraphQuery, HashSet<String>> latticeVisitedResultsOfStarkForGenerateNextBestMatch;

	public PriorityQueue<GraphResult> anyTimeResults;
	public double leastAnyTimeValueResult;
	int numberOfQueryEdges = 0;

	public ArrayList<GraphResult> finalResultsArrayList;

	public HashMap<StarGraphQuery, PriorityQueue<StarGraphResult>> starkForLeafPQResults;
	...
	public HashMap<Integer, TreeNode<CalculationNode>> calcTreeStarQueriesNodeMapBySQMLIndex;
	...
	public AnyTimeStarFramework copy(GraphDatabaseService queryGraph, GraphDatabaseService knowledgeGraph) {

		AnyTimeStarFramework newStarFramework = new AnyTimeStarFramework(queryGraph, knowledgeGraph, this.k, this.alpha,
				this.levenshtein);
		newStarFramework.numberOfQueryEdges = this.numberOfQueryEdges;
		newStarFramework.leastAnyTimeValueResult = this.leastAnyTimeValueResult;
		...
		// anytimeResultPQ:
		GraphResult[] anytimeArr = new GraphResult[this.anyTimeResults.size()];
		this.anyTimeResults.toArray(anytimeArr);
		for (int s = 0; s < anytimeArr.length; s++) {
			newStarFramework.anyTimeResults.add(anytimeArr[s]);
		}
		...
		for (Integer index : newStarFramework.calcTreeNodeMap.keySet()) {
			if (newStarFramework.calcTreeNodeMap.get(index).getData().isStarQuery) {
				newStarFramework.calcTreeStarQueriesNodeMapBySQMLIndex.put(
						this.calcTreeNodeMap.get(index).getData().starQuery.starQueryIndex + 1,
						newStarFramework.calcTreeNodeMap.get(index));
			}
		}
	...
	public AnyTimeStarFramework(GraphDatabaseService queryGraph, GraphDatabaseService knowledgeGraph, int k,
			float alpha, Levenshtein levenshtein) {
		...
		anyTimeResults = new PriorityQueue<GraphResult>(k, new AnytimeResultComparator());
		...
		calcTreeStarQueriesNodeMapBySQMLIndex = new HashMap<Integer, TreeNode<CalculationNode>>();
...

	{
		...
		for (StarGraphQuery sq : starQueries) {
			...
			numberOfQueryEdges += sq.allStarGraphQueryNodes.size() - 1;
	...
	public ArrayList<GraphResult> starRoundRobinRun(GraphDatabaseService queryGraph,
		...
		double sFetchTime, sJoinTime, sfinishCheckTime;
		...
		do {
			...
			if (allStarNodeIndicesSize > 0) {
				...
				if (!thisCalcNode.getData().callStarKIsEnough) {
					infoHolder.sfFetchesCalls++;
					thisCalcNode.getData().numberOfPartialAnswersShouldBeFetched = k;
					infoHolder.sfTotalRequestForFetches += k;

					anyTimeStarkForLeaf(knowledgeGraph, thisCalcNode, neighborIndexingInstance, cacheServer);
					depthJoinLevel = thisCalcNode.levelInCalcTree - 1;

				}
			}
			infoHolder.sfFetchesTime += ((System.nanoTime() - sFetchTime) / 1e6);

			sJoinTime = System.nanoTime();
			for (; depthJoinLevel >= 0; depthJoinLevel--) {
				infoHolder.sfJoinCalls++;
				CalculationTreeSiblingNodes calculationTreeSiblingNodes = joinLevelSiblingNodesMap.get(depthJoinLevel);
				anyTimeTwoWayHashJoin(calculationTreeSiblingNodes.leftNode, calculationTreeSiblingNodes.rightNode, k);
			}
			infoHolder.sfJoinTime += ((System.nanoTime() - sJoinTime) / 1e6);
			cnt++;
			// if ((cnt % 1000) == 0) {
			// System.out.println("cnt: " + cnt);
			// }
			// timeout
			// if (((System.nanoTime() - startTime) / 1e6) > 70000) {
			// this.timeOut = true;
			...
			sfinishCheckTime = System.nanoTime();
			shouldFinish = anyTimeAlgorithmShouldFinish();
			infoHolder.sfCheckShouldFinishTime += ((System.nanoTime() - sfinishCheckTime) / 1e6);

		}
		// check to see if the algorithm is finished?
		while (!shouldFinish);
		//
		// System.out.println("fetch nums: " + starkOperation);
		// System.out.println("join nums: " + joinOperation);

		return finalResultsArrayList;
	}

	public boolean anyTimeAlgorithmShouldFinish() {
		...
		if (anyTimeResults.size() >= k) {
			if (starQueries.size() == 1) {
				return true;
			}
			if (starQueries.size() > 1 && anyTimeSatisfactionOfAlphaSchema()) {
				return true;
			}
			return false;
		}
	...
	private void starRun(GraphDatabaseService knowledgeGraph, TreeNode<CalculationNode> rootTreeCalculationNode,
		...
		while (!rootTreeCalculationNode.getData().callStarKIsEnough) {
			...
			if (anyTimeAlgorithmShouldFinish()) {
				break;
			}
	...
	private ArrayList<StarGraphQuery> sortStarQueriesForCalcTree(ArrayList<StarGraphQuery> starQueries) {
		...
		for (int i = 0; i < starQueries.size(); i++) {
			starQueries.get(i).starQueryIndex = i;
		}
		return starQueries;
	}
	...
	private TreeNode<CalculationNode> GenerateCalculationTree(GraphDatabaseService queryGraph,
		...
		for (Integer index : calcTreeNodeMap.keySet()) {
			if (calcTreeNodeMap.get(index).getData().isStarQuery) {
				calcTreeStarQueriesNodeMapBySQMLIndex.put(
						calcTreeNodeMap.get(index).getData().starQuery.starQueryIndex + 1, calcTreeNodeMap.get(index));
			}
		}
		return rootTreeNode;
	}
	...
	private void stark(GraphDatabaseService knowledgeGraph, TreeNode<CalculationNode> calculationNode,
		...
		if (calculationNode.isLeaf()) {
			...
			boolean hasResult = anyTimeStarkForLeaf(knowledgeGraph, calculationNode, neighborIndexingInstance,
					cacheServer);
	...
	public void starjoin(GraphDatabaseService knowledgeGraph, TreeNode<CalculationNode> leftChild,
		...
		if (leftChild.isLeaf() || leftChild.getData().hasAtLeastOneUnvisitedReuslt()
			...
			anyTimeTwoWayHashJoin(leftChild, rightChild, k);

		}

	}

	private boolean anyTimeSatisfactionOfAlphaSchema() {
		// when we come here we have at least k answers
		boolean shouldFinish = true;

//		if (calcTreeNodeMap.get(1).getData().depthOfDigging > 1800 && calcTreeNodeMap.get(2).getData().depthOfDigging > 1800)
//			System.out.println();

		for (Integer nodeIndex : calcTreeNodeMap.keySet()) {
			if (!calcTreeNodeMap.get(nodeIndex).getData().callStarKIsEnough
					&& calcTreeNodeMap.get(nodeIndex).getData().anytimeUpperBound > leastAnyTimeValueResult) {
				shouldFinish = false;
				break;
			}
		}
//		if (shouldFinish) {
//
//			System.out.println("callStarKIsEnough:");
//			for (Integer nodeIndex : calcTreeNodeMap.keySet()) {
//				System.out.print(calcTreeNodeMap.get(nodeIndex).getData().callStarKIsEnough + ", ");
//			}
//
//			System.out.println();
//
//			System.out.println("depthOfDigging1:");
//			for (Integer nodeIndex : calcTreeNodeMap.keySet()) {
//				System.out.print(calcTreeNodeMap.get(nodeIndex).getData().depthOfDigging + ", ");
//			}
//			System.out.println();
//
//			System.out.println("depthOfDigging2:");
//			for (Integer nodeIndex : calcTreeStarQueriesNodeMap.keySet()) {
//				System.out.print(calcTreeStarQueriesNodeMap.get(nodeIndex).getData().depthOfDigging + ", ");
//			}
//			System.out.println();
//
//			System.out.println("upperbound checking:");
//			for (Integer nodeIndex : calcTreeNodeMap.keySet()) {
//				System.out.print(calcTreeNodeMap.get(nodeIndex).getData().anytimeUpperBound + " >? "
//						+ leastAnyTimeValueResult + " => "
//						+ (calcTreeNodeMap.get(nodeIndex).getData().anytimeUpperBound > leastAnyTimeValueResult)
//						+ " \n");
//
//			}
...
//		}
	...
	public void anyTimeTwoWayHashJoin(TreeNode<CalculationNode> leftChild, TreeNode<CalculationNode> rightChild,
			int k) {
		...
		if (forParentResult != null && forParentResult.size() > 0) {
			...
			for (GraphResult gr : forParentResult) {
				leftChild.getParent().getData().graphResult.add(gr);
				checkAnyTimeResult(leftChild.getParent().getData(), gr);
			}

		}

		lNodeData.anytimeUpperBoundUpdate(lNodeData, rNodeData, leftChild.getParent().getData());
	...
	private void checkAnyTimeResult(CalculationNode data, GraphResult gr) {
		gr.anyTimeItemValue = gr.totalValue + data.numberOfNodes + data.numberOfEdges;

		if (anyTimeResults.size() < k) {
			anyTimeResults.add(gr);
			leastAnyTimeValueResult = anyTimeResults.peek().anyTimeItemValue;
		} else {
			if (gr.anyTimeItemValue > leastAnyTimeValueResult) {
				anyTimeResults.poll();
				anyTimeResults.add(gr);
				leastAnyTimeValueResult = anyTimeResults.peek().anyTimeItemValue;
			}
	...
	public boolean anyTimeStarkForLeaf(GraphDatabaseService knowledgeGraph, TreeNode<CalculationNode> treeNode,
			NeighborIndexing neighborIndexingInstance, CacheServer cacheServer) {
		...
		calcNodeData.howManyTimesSelectedBefore++;
		DummyFunctions
		...
		for (StarGraphResult starGraphResult : resultStarGraphs) {
			...
			if (treeNode.getParent() != null) {
				gr = new GraphResult(assembledResult, treeNode.getParent().getData().joinableQueryGraphNodes,
						starQueryIndexDepthMap);
			} else {
				gr = new GraphResult(assembledResult, null, starQueryIndexDepthMap);
			}
			calcNodeData.graphResult.add(gr);
			checkAnyTimeResult(calcNodeData, gr);

		}
		// TODO: if call stark is enough we don't need to update this
		calcNodeData.updateLastAndFirstItems(queryNodeIntersectMap);
	...
	public ArrayList<StarGraphResult> findTopOneMatchesPivotedAtVInG(GraphDatabaseService knowledgeGraph,
		...
		for (Long gpNodeId : possibleNodeIdSet) {
			...
			if ((topOneResults.size() / 1000000) >= 1) {
